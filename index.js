...
app.get('/', async (req, res) => {
  ...
  const body = {
    properties: ['full_name', 'favorite_color', 'bio'],
    limit: 100,
    sorts: [
      {
  ...
  try {
    const resp = await axios.post(searchEndpoint, body, { headers });
    const contacts = resp.data.results;
    res.render('homepage', { title: 'Contacts List', contacts });
  } catch (error) {
    console.error(error);
...
app.post('/update-cobj', async (req, res) => {
  const createContact = 'https://api.hubapi.com/crm/v3/objects/contacts';
  const headers = {
    Authorization: `Bearer ${PRIVATE_APP_ACCESS}`,
    'Content-Type': 'application/json'
  };
